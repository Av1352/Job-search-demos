...
def generate_credential_data():
    ...
    for i in range(50):
        ...
        is_valid = np.random.random() < 0.92  # 92% valid
...
def process_dashboard():
    """Generate complete verification dashboard"""
    df = generate_credential_data()
    total_verifications, valid_count, success_rate, avg_time, by_type = analyze_verifications(df)
    
    verification_chart = create_verification_chart(by_type)
    time_chart = create_time_distribution(df)
    ...
    return summary_html, verification_chart, time_chart, df
...
with gr.Blocks(css=custom_css, theme=gr.themes.Soft()) as demo:
    ...
    with gr.Tabs():
        with gr.Tab("📊 Verification Analytics"):
            gr.HTML("""
            ...
            dashboard_btn = gr.Button("🔍 Load Analytics", variant="primary", size="lg")
            
            summary_md = gr.HTML()
            
            with gr.Row():
                with gr.Column():
                    verification_chart = gr.Plot(label="Success Rate by Type")
                with gr.Column():
                    time_chart = gr.Plot(label="Verification Time Distribution")
            
            verification_table = gr.Dataframe(label="Recent Verifications")
        
        with gr.Tab("🔐 Verify Credential"):
            gr.HTML("""
            ...
            with gr.Row():
                with gr.Column():
                    cred_type_input = gr.Dropdown(
                        choices=['Driver License', 'Passport', 'National ID', 'Birth Certificate', 
                                'University Degree', 'Employment Record', 'Medical License', 'Voter ID'],
                        label="Credential Type",
                        value='Driver License'
                    )
                    issuer_input = gr.Textbox(label="Issuing Authority", value="DMV California")
                    holder_input = gr.Textbox(label="Credential Holder", value="John Doe")
                    verify_btn = gr.Button("✓ Verify Credential", variant="primary", size="lg")
            
            verification_output = gr.HTML()
    
    dashboard_btn.click(
        fn=process_dashboard,
        outputs=[summary_md, verification_chart, time_chart, verification_table]
    )
    
    verify_btn.click(
        fn=verify_credential,
        inputs=[cred_type_input, issuer_input, holder_input],
    ...
    <div style="text-align: center; padding: 40px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 20px; box-shadow: 0 12px 28px rgba(102, 126, 234, 0.35); color: white;">
        ...
        <p style="font-size: 15px; margin: 18px 0; font-weight: 700;">
            <strong style="color: white;">Tech Stack:</strong> Python • SHA-256 • Digital Signatures • Plotly • Gradio
        </p>
