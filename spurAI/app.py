...
def simulate_shopper_behavior(persona, scenario, num_sessions):
    ...
    for variant in test_scenario['variants']:
        # Base conversion from persona
        base_conversion = shopper['conversion_rate']
        
        # Variant impact (simulate different performance)
    ...
    results_html += """
        ...
        <div style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); border-radius: 12px; padding: 20px; margin-top: 20px; color: white;">
            ...
            <p style="font-size: 14px; margin: 0;">Deploy this variant to increase conversion by """ + f"{((winner['conversion_rate'] - results_sorted[-1]['conversion_rate']) / results_sorted[-1]['conversion_rate'] * 100):.0f}%" + """ vs worst performer</p>
        </div>
    </div>
    """
    ...
    return summary_html + results_html + persona_html, fig_cvr, fig_revenue
...
def generate_ab_test_results():
    ...
    return dashboard_html + tests_html, fig_trends
...
with gr.Blocks(css=custom_css, theme=gr.themes.Soft()) as demo:
        ...
        with gr.Tab("🛍️ Run Simulation"):
            ...
            with gr.Row():
                persona_dropdown = gr.Dropdown(
                    choices=list(SHOPPER_PERSONAS.keys()),
                    value="Budget Hunter",
                ...
                scenario_dropdown = gr.Dropdown(
                    ...
                    value="Product Page Optimization",
                    label="Test Scenario"
                )
            ...
            num_sessions = gr.Slider(
                ...
                value=1000,
                step=100,
                label="Number of Simulated Sessions"
            )
            
            simulate_btn = gr.Button("🚀 Run Simulation", variant="primary", size="lg")
            
            simulation_output = gr.HTML(label="Simulation Results")
            cvr_chart = gr.Plot(label="Conversion Rate Comparison")
            revenue_chart = gr.Plot(label="Revenue Analysis")
            
            simulate_btn.click(
                fn=simulate_shopper_behavior,
                inputs=[persona_dropdown, scenario_dropdown, num_sessions],
                outputs=[simulation_output, cvr_chart, revenue_chart]
            )
        
        with gr.Tab("📊 Test Dashboard"):
            gr.HTML("""
            ...
            trend_chart = gr.Plot(label="Testing Activity")
            
            dashboard_btn.click(
                fn=generate_ab_test_results,
                inputs=[],
    ...
    <div style="text-align: center; padding: 40px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 20px; box-shadow: 0 12px 28px rgba(102, 126, 234, 0.35); color: white;">
        ...
        <p style="font-size: 15px; margin: 18px 0; font-weight: 700;">
            <strong style="color: white;">Tech Stack:</strong> Python • Gradio • Simulation • E-commerce Analytics
        </p>
