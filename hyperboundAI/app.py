...
def analyze_sales_call(call_name):
    ...
    # Scoring
    talk_ratio_score = 100 - abs(call['talk_ratio'] - 30) * 2  # Ideal: 30% rep talk
...
with gr.Blocks(css=custom_css, theme=gr.themes.Soft()) as demo:
    ...
    with gr.Tabs():
        with gr.Tab("📞 Analyze Call"):
            ...
            call_dropdown = gr.Dropdown(
                choices=list(SAMPLE_CALLS.keys()),
                value="Discovery Call - Tech SaaS",
                label="Select Call Recording"
            )
            
            analyze_btn = gr.Button("🎯 Analyze Call Performance", variant="primary", size="lg")
            
            analysis_output = gr.HTML(label="Call Analysis")
            performance_chart = gr.Plot(label="Performance Radar")
            
            analyze_btn.click(
                fn=analyze_sales_call,
                inputs=[call_dropdown],
                outputs=[analysis_output, performance_chart]
            )
        ...
        with gr.Tab("📊 Team Dashboard"):
            ...
            dashboard_btn = gr.Button("📊 Load Team Dashboard", variant="primary", size="lg")
            
            dashboard_output = gr.HTML(label="Dashboard")
            trend_chart = gr.Plot(label="Win Rate Trend")
            
            dashboard_btn.click(
                fn=generate_team_dashboard,
                inputs=[],
    ...
    <div style="text-align: center; padding: 40px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 20px; box-shadow: 0 12px 28px rgba(102, 126, 234, 0.35); color: white;">
        ...
        <p style="font-size: 15px; margin: 18px 0; font-weight: 700;">
            <strong style="color: white;">Tech Stack:</strong> Python • Gradio • NLP • Sales Analytics
        </p>
