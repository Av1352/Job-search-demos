...
def generate_clinical_data(n_samples=50):
    ...
    for i in range(n_samples):
        ...
        # Vital signs with occasional anomalies
        ...
        # Introduce some anomalies (10% of data)
...
def validate_clinical_data():
    ...
    <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 20px; padding: 32px; box-shadow: 0 8px 20px rgba(102, 126, 234, 0.3); margin-bottom: 25px;">
        <h2 style="color: white; font-size: 32px; font-weight: 900; margin: 0 0 20px 0;">🔬 GxP Data Validation Results</h2>
        ...
        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px;">
            <div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border-radius: 16px; padding: 22px; text-align: center; border: 2px solid rgba(255,255,255,0.2);">
                <p style="font-size: 14px; color: rgba(255,255,255,0.8); margin: 0 0 10px 0; font-weight: 600;">Quality Score</p>
                <p style="font-size: 48px; color: {status_color}; font-weight: 900; margin: 0; text-shadow: 0 2px 4px rgba(0,0,0,0.2);">{overall_score:.0f}%</p>
                <p style="font-size: 13px; color: rgba(255,255,255,0.7); margin: 8px 0 0 0;">{status_emoji} {status}</p>
            </div>
            
            <div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border-radius: 16px; padding: 22px; text-align: center; border: 2px solid rgba(255,255,255,0.2);">
                <p style="font-size: 14px; color: rgba(255,255,255,0.8); margin: 0 0 10px 0; font-weight: 600;">Records Validated</p>
                <p style="font-size: 48px; color: white; font-weight: 900; margin: 0; text-shadow: 0 2px 4px rgba(0,0,0,0.2);">{total_records}</p>
                <p style="font-size: 13px; color: rgba(255,255,255,0.7); margin: 8px 0 0 0;">Clinical trial data</p>
            </div>
            
            <div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border-radius: 16px; padding: 22px; text-align: center; border: 2px solid rgba(255,255,255,0.2);">
                <p style="font-size: 14px; color: rgba(255,255,255,0.8); margin: 0 0 10px 0; font-weight: 600;">Missing Data</p>
                <p style="font-size: 48px; color: {'#fca5a5' if missing_data_count > 0 else '#86efac'}; font-weight: 900; margin: 0; text-shadow: 0 2px 4px rgba(0,0,0,0.2);">{missing_data_count}</p>
                <p style="font-size: 13px; color: rgba(255,255,255,0.7); margin: 8px 0 0 0;">{missing_data_count/total_records*100:.1f}% incomplete</p>
            </div>
            
            <div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border-radius: 16px; padding: 22px; text-align: center; border: 2px solid rgba(255,255,255,0.2);">
                <p style="font-size: 14px; color: rgba(255,255,255,0.8); margin: 0 0 10px 0; font-weight: 600;">Outliers Detected</p>
                <p style="font-size: 48px; color: #fbbf24; font-weight: 900; margin: 0; text-shadow: 0 2px 4px rgba(0,0,0,0.2);">{outlier_count}</p>
                <p style="font-size: 13px; color: rgba(255,255,255,0.7); margin: 8px 0 0 0;">{outlier_count/total_records*100:.1f}% flagged</p>
            </div>
        </div>
    </div>
    """
    ...
    <div style="background: linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 100%); border: 3px solid #3b82f6; border-radius: 20px; padding: 28px; box-shadow: 0 8px 20px rgba(59, 130, 246, 0.2); margin-bottom: 25px;">
        <h3 style="color: #1e40af; font-size: 26px; font-weight: 900; margin: 0 0 20px 0;">📋 ALCOA+ Compliance Check</h3>
        <p style="color: #3b82f6; font-size: 14px; margin: 0 0 20px 0; font-weight: 600;">FDA 21 CFR Part 11 Data Integrity Principles</p>
    ...
    for principle, (passed, detail) in alcoa_checks.items():
        ...
        alcoa_html += f"""
        <div style="background: white; border-left: 5px solid {check_color}; border-radius: 12px; padding: 16px; box-shadow: 0 2px 6px rgba(0,0,0,0.08);">
            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                <div style="background: {check_color}; width: 30px; height: 30px; border-radius: 50%; display: flex; align-items: center; justify-content: center;">
                    <span style="font-size: 16px; color: white; font-weight: 900;">{check_icon}</span>
                </div>
                <p style="font-size: 16px; color: #1f2937; font-weight: 800; margin: 0;">{principle}</p>
            </div>
            <p style="font-size: 13px; color: #6b7280; margin: 0; padding-left: 40px;">{detail}</p>
        </div>
        """
    ...
    if missing_data_count > 0 or outlier_count > 0:
        ...
        <div style="background: linear-gradient(135deg, #fee2e2 0%, #fecaca 100%); border: 3px solid #ef4444; border-radius: 20px; padding: 28px; box-shadow: 0 8px 20px rgba(239, 68, 68, 0.2); margin-bottom: 25px;">
            <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 20px;">
                ...
                <div>
                    <h3 style="color: #991b1b; font-size: 26px; font-weight: 900; margin: 0;">Data Quality Issues</h3>
                    <p style="color: #dc2626; font-size: 16px; margin: 6px 0 0 0; font-weight: 600;">{missing_data_count + outlier_count} issues require attention</p>
                </div>
            </div>
        ...
        if missing_data_count > 0:
            ...
            <div style="background: white; border-left: 5px solid #ef4444; border-radius: 12px; padding: 18px; box-shadow: 0 2px 6px rgba(0,0,0,0.08);">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div>
                        <span style="background: #dc2626; color: white; padding: 4px 12px; border-radius: 12px; font-size: 11px; font-weight: 800; margin-right: 10px;">CRITICAL</span>
                        <span style="font-size: 18px; color: #1f2937; font-weight: 700;">Incomplete Records</span>
                    </div>
                    <p style="font-size: 28px; color: #dc2626; font-weight: 900; margin: 0;">{missing_data_count}</p>
                </div>
                <p style="font-size: 14px; color: #6b7280; margin: 10px 0 0 0;">Missing vital signs data - violates ALCOA+ "Complete" principle</p>
                <div style="background: #fef3c7; border-radius: 8px; padding: 12px; margin-top: 12px;">
                    <p style="font-size: 13px; color: #92400e; font-weight: 600; margin: 0;">🔧 Action: Query source system, request data re-entry, document deviation</p>
                </div>
            </div>
            """
        ...
        if outlier_count > 0:
            issues_html += f"""
            <div style="background: white; border-left: 5px solid #f59e0b; border-radius: 12px; padding: 18px; box-shadow: 0 2px 6px rgba(0,0,0,0.08);">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div>
                        <span style="background: #f59e0b; color: white; padding: 4px 12px; border-radius: 12px; font-size: 11px; font-weight: 800; margin-right: 10px;">HIGH</span>
                        <span style="font-size: 18px; color: #1f2937; font-weight: 700;">Statistical Outliers</span>
                    </div>
                    <p style="font-size: 28px; color: #f59e0b; font-weight: 900; margin: 0;">{outlier_count}</p>
                </div>
                <p style="font-size: 14px; color: #6b7280; margin: 10px 0 0 0;">Values outside normal clinical ranges - potential data entry errors or true adverse events</p>
                <div style="background: #fef3c7; border-radius: 8px; padding: 12px; margin-top: 12px;">
                    <p style="font-size: 13px; color: #92400e; font-weight: 600; margin: 0;">🔧 Action: Clinical review required, verify with source documents, flag for monitoring</p>
                </div>
            </div>
            """
        ...
        <div style="background: linear-gradient(135deg, #d1fae5 0%, #a7f3d0 100%); border: 3px solid #10b981; border-radius: 20px; padding: 28px; box-shadow: 0 8px 20px rgba(16, 185, 129, 0.2); margin-bottom: 25px;">
            <div style="display: flex; align-items: center; gap: 15px;">
                ...
                <div>
                    <h3 style="color: #065f46; font-size: 26px; font-weight: 900; margin: 0;">All Validation Checks Passed</h3>
                    <p style="color: #047857; font-size: 16px; margin: 6px 0 0 0; font-weight: 600;">Data meets all GxP compliance requirements</p>
                </div>
            </div>
        </div>
        """
    ...
    return summary_html + alcoa_html + issues_html, fig_quality, fig_outliers, df
...
def generate_audit_trail():
    ...
    <div style="background: linear-gradient(135deg, #f3e8ff 0%, #e9d5ff 100%); border: 3px solid #a855f7; border-radius: 20px; padding: 28px; box-shadow: 0 8px 20px rgba(168, 85, 247, 0.2); margin-bottom: 25px;">
        <h3 style="color: #6b21a8; font-size: 26px; font-weight: 900; margin: 0 0 20px 0;">📜 Audit Trail (FDA 21 CFR Part 11)</h3>
        <p style="color: #a855f7; font-size: 14px; margin: 0 0 20px 0; font-weight: 600;">Complete record of all data modifications with electronic signatures</p>
    ...
    for i in range(8):
        ...
        audit_html += f"""
        <div style="background: white; border-radius: 12px; padding: 18px; box-shadow: 0 2px 6px rgba(0,0,0,0.08); border-left: 4px solid #a855f7;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                <div>
                    <span style="background: #a855f7; color: white; padding: 4px 12px; border-radius: 12px; font-size: 11px; font-weight: 800; margin-right: 8px;">{action.upper()}</span>
                    <span style="font-size: 15px; color: #1f2937; font-weight: 700;">Record #{i+1}</span>
                </div>
                <p style="font-size: 13px; color: #6b7280; margin: 0; font-family: monospace;">{timestamp.strftime('%Y-%m-%d %H:%M:%S')}</p>
            </div>
            
            <div style="background: #f9fafb; border-radius: 8px; padding: 12px; margin-bottom: 10px;">
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px;">
                    <div>
                        <p style="font-size: 11px; color: #6b7280; margin: 0 0 4px 0;">User</p>
                        <p style="font-size: 13px; color: #1f2937; font-weight: 600; margin: 0; font-family: monospace;">{user}</p>
                    </div>
                    <div>
                        <p style="font-size: 11px; color: #6b7280; margin: 0 0 4px 0;">Old Value</p>
                        <p style="font-size: 13px; color: #ef4444; font-weight: 600; margin: 0;">{old_val}</p>
                    </div>
                    <div>
                        <p style="font-size: 11px; color: #6b7280; margin: 0 0 4px 0;">New Value</p>
                        <p style="font-size: 13px; color: #10b981; font-weight: 600; margin: 0;">{new_val}</p>
                    </div>
                </div>
            </div>
            
            <p style="font-size: 13px; color: #6b7280; margin: 0;"><strong>Reason:</strong> {reason}</p>
        </div>
        """
    ...
    audit_html += """
        ...
        <div style="background: linear-gradient(135deg, #a855f7 0%, #9333ea 100%); border-radius: 12px; padding: 20px; margin-top: 20px; color: white;">
            <p style="font-size: 16px; font-weight: 800; margin: 0 0 10px 0;">✅ Audit Trail Compliance Status</p>
            <ul style="margin: 0; padding-left: 24px; line-height: 2;">
                <li style="font-size: 14px;">All changes logged with unique user ID</li>
                <li style="font-size: 14px;">Timestamps recorded for every modification</li>
                <li style="font-size: 14px;">Old and new values captured</li>
                <li style="font-size: 14px;">Reason for change documented</li>
                <li style="font-size: 14px;">Electronic signatures verified</li>
            </ul>
        </div>
    </div>
    """
...
with gr.Blocks(css=custom_css, theme=gr.themes.Soft()) as demo:
    ...
    with gr.Tabs():
        with gr.Tab("🔬 Data Validation"):
            ...
            validate_btn = gr.Button("🔍 Validate Clinical Data", variant="primary", size="lg")
            
            validation_output = gr.HTML(label="Validation Results")
            quality_chart = gr.Plot(label="Quality Scores")
            outlier_chart = gr.Plot(label="Outlier Detection")
            data_table = gr.Dataframe(label="Clinical Data Sample", wrap=True)
            
            validate_btn.click(
                fn=validate_clinical_data,
                inputs=[],
                outputs=[validation_output, quality_chart, outlier_chart, data_table]
            )
        ...
        with gr.Tab("📜 Audit Trail"):
            ...
            audit_btn = gr.Button("📜 Generate Audit Trail", variant="primary", size="lg")
            
            audit_output = gr.HTML(label="Audit Trail")
            audit_chart = gr.Plot(label="Audit Activity")
            
            audit_btn.click(
                fn=generate_audit_trail,
                inputs=[],
                outputs=[audit_output, audit_chart]
            )
    ...
    <div style="text-align: center; padding: 40px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 20px; box-shadow: 0 12px 28px rgba(102, 126, 234, 0.35); color: white;">
        ...
        <p style="font-size: 15px; margin: 18px 0; font-weight: 700;">
            <strong style="color: white;">Tech Stack:</strong> Python • Gradio • Plotly • Pandas • Statistical Analysis
        </p>
