...
def process_dashboard():
    ...
    success_chart = create_success_rate_chart(success_by_method)
    processing_chart = create_processing_time_chart(avg_processing)
    ...
    return summary_html, success_chart, processing_chart, df
...
with gr.Blocks(css=custom_css, theme=gr.themes.Soft()) as demo:
    ...
    with gr.Tabs():
        ...
        with gr.Tab("🎯 Payment Router"):
            ...
            with gr.Row():
                with gr.Column():
                    amount_input = gr.Number(label="Transaction Amount ($)", value=1000, minimum=1)
                    method_input = gr.Dropdown(
                        choices=['Credit Card', 'Debit Card', 'ACH', 'Wire Transfer', 'Digital Wallet'],
                        label="Payment Method",
                        value='Credit Card'
                    )
                    route_btn = gr.Button("🚀 Get Optimal Route", variant="primary", size="lg")
            
            routing_output = gr.HTML()
    
    dashboard_btn.click(
        fn=process_dashboard,
        outputs=[summary_md, success_chart, processing_chart, transaction_table]
    )
    
    route_btn.click(
        fn=optimize_routing,
        inputs=[amount_input, method_input],
    ...
    <div style="text-align: center; padding: 40px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 20px; box-shadow: 0 12px 28px rgba(102, 126, 234, 0.35); color: white;">
        ...
        <p style="font-size: 15px; margin: 18px 0; font-weight: 700;">
            <strong style="color: white;">Tech Stack:</strong> Python • Gradio • Routing Algorithms • Real-time Analytics
        </p>
