...
def generate_payoff_plan(df):
    ...
    for idx, (_, row) in enumerate(sorted_debt.head(5).iterrows()):
        ...
        card = f'<div style="background: white; border-left: 5px solid {colors[idx]}; border-radius: 12px; padding: 20px; box-shadow: 0 2px 6px rgba(0,0,0,0.08);"><div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 12px;"><div><p style="font-size: 20px; color: #1f2937; font-weight: 800; margin: 0 0 6px 0;">Priority #{idx+1}: {row["type"]}</p><p style="font-size: 13px; color: #6b7280; margin: 0;">Account: {row["id"]}</p></div><div style="background: {colors[idx]}; color: white; padding: 6px 12px; border-radius: 8px;"><p style="font-size: 14px; font-weight: 800; margin: 0;">{row["interest_rate"]}% APR</p></div></div><div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; background: #f9fafb; border-radius: 10px; padding: 14px;"><div><p style="font-size: 11px; color: #6b7280; margin: 0 0 4px 0;">Balance</p><p style="font-size: 18px; color: #1f2937; font-weight: 800; margin: 0;">${row["balance"]:,.0f}</p></div><div><p style="font-size: 11px; color: #6b7280; margin: 0 0 4px 0;">Monthly</p><p style="font-size: 18px; color: #1f2937; font-weight: 800; margin: 0;">${row["monthly_payment"]:.0f}</p></div><div><p style="font-size: 11px; color: #6b7280; margin: 0 0 4px 0;">Payoff</p><p style="font-size: 18px; color: #1f2937; font-weight: 800; margin: 0;">{months_to_payoff:.0f}mo</p></div><div><p style="font-size: 11px; color: #6b7280; margin: 0 0 4px 0;">Interest</p><p style="font-size: 18px; color: #ef4444; font-weight: 800; margin: 0;">${total_interest:,.0f}</p></div></div></div>'
        plan_cards.append(card)
    
    all_cards = ''.join(plan_cards)
    
    plan_html = f'<div style="background: linear-gradient(135deg, #d1fae5 0%, #a7f3d0 100%); border: 3px solid #10b981; border-radius: 20px; padding: 28px; box-shadow: 0 8px 20px rgba(16, 185, 129, 0.2); margin-bottom: 25px;"><h3 style="color: #065f46; font-size: 26px; font-weight: 900; margin: 0 0 20px 0;">💡 Optimal Payoff Strategy (Avalanche Method)</h3><p style="color: #059669; font-size: 16px; margin: 0 0 20px 0; font-weight: 600;">Pay highest interest debts first to minimize total interest paid</p><div style="display: grid; gap: 12px;">{all_cards}</div><div style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); border-radius: 12px; padding: 20px; margin-top: 18px; text-align: center; color: white;"><p style="font-size: 20px; font-weight: 900; margin: 0;">💰 Total Interest to Pay: ${total_interest_saved:,.0f}</p><p style="font-size: 14px; margin: 8px 0 0 0; opacity: 0.9;">Following this priority order minimizes your total interest</p></div></div>'
...
def calculate_consolidation_savings(df):
    ...
    result_html = f'<div style="background: linear-gradient(135deg, #{"d1fae5" if should_consolidate else "fef3c7"} 0%, #{"a7f3d0" if should_consolidate else "fde68a"} 100%); border: 3px solid #{"10b981" if should_consolidate else "f59e0b"}; border-radius: 20px; padding: 28px; box-shadow: 0 8px 20px rgba({"16, 185, 129" if should_consolidate else "245, 158, 11"}, 0.2); margin-bottom: 25px;"><h3 style="color: #{"065f46" if should_consolidate else "92400e"}; font-size: 26px; font-weight: 900; margin: 0 0 20px 0;">🎯 Debt Consolidation Analysis</h3><div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px; margin-bottom: 20px;"><div style="background: white; border-radius: 12px; padding: 20px; text-align: center;"><p style="font-size: 13px; color: #6b7280; margin: 0 0 8px 0;">Current Annual Interest</p><p style="font-size: 36px; color: #ef4444; font-weight: 900; margin: 0;">${current_interest:,.0f}</p></div><div style="background: white; border-radius: 12px; padding: 20px; text-align: center;"><p style="font-size: 13px; color: #6b7280; margin: 0 0 8px 0;">Consolidated Interest ({consolidation_rate}%)</p><p style="font-size: 36px; color: #10b981; font-weight: 900; margin: 0;">${consolidated_interest:,.0f}</p></div></div><div style="background: white; border-radius: 12px; padding: 24px; margin-bottom: 20px;"><div style="text-align: center;"><p style="font-size: 14px; color: #6b7280; margin: 0 0 10px 0;">Annual Savings</p><p style="font-size: 48px; color: #10b981; font-weight: 900; margin: 0;">${annual_savings:,.0f}</p><p style="font-size: 16px; color: #6b7280; margin: 10px 0 0 0;">${monthly_savings:,.0f}/month</p></div></div><div style="background: linear-gradient(135deg, #{"10b981" if should_consolidate else "f59e0b"} 0%, #{"059669" if should_consolidate else "ea580c"} 100%); border-radius: 12px; padding: 20px; text-align: center; color: white;"><p style="font-size: 24px; font-weight: 900; margin: 0 0 10px 0;">{"✅ RECOMMENDED: CONSOLIDATE" if should_consolidate else "⚠️ REVIEW CAREFULLY"}</p><p style="font-size: 15px; margin: 0; opacity: 0.95;">{"Significant savings with lower interest rate" if should_consolidate else "Marginal savings - consider other factors"}</p></div><div style="background: rgba(255,255,255,0.8); border-radius: 10px; padding: 18px; margin-top: 15px;"><p style="font-size: 15px; color: #1f2937; font-weight: 700; margin: 0 0 10px 0;">Benefits of Consolidation:</p><div style="display: grid; gap: 8px;"><p style="font-size: 14px; color: #4b5563; margin: 0;">✓ Single monthly payment (simplified management)</p><p style="font-size: 14px; color: #4b5563; margin: 0;">✓ Lower average interest rate</p><p style="font-size: 14px; color: #4b5563; margin: 0;">✓ Potential credit score improvement</p><p style="font-size: 14px; color: #4b5563; margin: 0;">✓ Faster payoff timeline</p></div></div></div>'
...
def process_dashboard():
    ...
    return total_debt, monthly_payment, avg_interest, high_risk, health_status, health_color, breakdown_chart, risk_chart, payoff_plan, consolidation, df
...
# Tabs
tab1, tab2, tab3 = st.tabs(["📊 Portfolio Analysis", "🎯 Payoff Strategy", "💡 Consolidation Analysis"])
...
with tab1:
    ...
    if st.session_state.portfolio_analyzed:
        total_debt, monthly_payment, avg_interest, high_risk, health_status, health_color, breakdown_chart, risk_chart, payoff_plan, consolidation, df = process_dashboard()
        
        st.markdown(f'<div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 20px; padding: 32px; box-shadow: 0 8px 20px rgba(102, 126, 234, 0.3); margin-bottom: 25px;"><h2 style="color: white; font-size: 32px; font-weight: 900; margin: 0 0 20px 0;">💰 Consumer Liability Overview</h2><div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px;"><div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border-radius: 16px; padding: 22px; text-align: center; border: 2px solid rgba(255,255,255,0.2);"><p style="font-size: 14px; color: rgba(255,255,255,0.8); margin: 0 0 10px 0; font-weight: 600;">Total Debt</p><p style="font-size: 48px; color: white; font-weight: 900; margin: 0; text-shadow: 0 2px 4px rgba(0,0,0,0.2);">${total_debt/1000:.0f}K</p><p style="font-size: 13px; color: rgba(255,255,255,0.7); margin: 8px 0 0 0;">Outstanding</p></div><div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border-radius: 16px; padding: 22px; text-align: center; border: 2px solid rgba(255,255,255,0.2);"><p style="font-size: 14px; color: rgba(255,255,255,0.8); margin: 0 0 10px 0; font-weight: 600;">Monthly Payment</p><p style="font-size: 48px; color: #fbbf24; font-weight: 900; margin: 0; text-shadow: 0 2px 4px rgba(0,0,0,0.2);">${monthly_payment:,.0f}</p><p style="font-size: 13px; color: rgba(255,255,255,0.7); margin: 8px 0 0 0;">Total/month</p></div><div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border-radius: 16px; padding: 22px; text-align: center; border: 2px solid rgba(255,255,255,0.2);"><p style="font-size: 14px; color: rgba(255,255,255,0.8); margin: 0 0 10px 0; font-weight: 600;">Avg Interest</p><p style="font-size: 48px; color: white; font-weight: 900; margin: 0; text-shadow: 0 2px 4px rgba(0,0,0,0.2);">{avg_interest:.1f}%</p><p style="font-size: 13px; color: rgba(255,255,255,0.7); margin: 8px 0 0 0;">APR</p></div><div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border-radius: 16px; padding: 22px; text-align: center; border: 2px solid rgba(255,255,255,0.2);"><p style="font-size: 14px; color: rgba(255,255,255,0.8); margin: 0 0 10px 0; font-weight: 600;">High Risk</p><p style="font-size: 48px; color: {health_color}; font-weight: 900; margin: 0; text-shadow: 0 2px 4px rgba(0,0,0,0.2);">{high_risk}</p><p style="font-size: 13px; color: rgba(255,255,255,0.7); margin: 8px 0 0 0;">Liabilities</p></div></div><div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border-radius: 12px; padding: 18px; margin-top: 20px; text-align: center; border: 2px solid rgba(255,255,255,0.2);"><p style="font-size: 20px; color: white; font-weight: 800; margin: 0;">Portfolio Health: {health_status}</p><p style="font-size: 14px; color: rgba(255,255,255,0.8); margin: 8px 0 0 0;">Debt-to-Payment Ratio: {total_debt/monthly_payment:.1f} months</p></div></div>', unsafe_allow_html=True)
...
with tab2:
    if st.session_state.portfolio_analyzed:
        _, _, _, _, _, _, _, _, payoff_plan, _, _ = process_dashboard()
        st.markdown(payoff_plan, unsafe_allow_html=True)
    else:
        st.info("👈 Please analyze the portfolio first in the Portfolio Analysis tab")

with tab3:
    if st.session_state.portfolio_analyzed:
        _, _, _, _, _, _, _, _, _, consolidation, _ = process_dashboard()
        st.markdown(consolidation, unsafe_allow_html=True)
    else:
        st.info("👈 Please analyze the portfolio first in the Portfolio Analysis tab")
...
st.markdown("""
    ...
    <div style="text-align: center; padding: 40px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 20px; box-shadow: 0 12px 28px rgba(102, 126, 234, 0.35); color: white;">
        ...
        <p style="font-size: 15px; margin: 18px 0; font-weight: 700;">
            <strong style="color: white;">Tech Stack:</strong> Python • Pandas • Plotly • Optimization Algorithms • Streamlit
        </p>
