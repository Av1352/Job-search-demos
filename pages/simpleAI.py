...
st.set_page_config(page_title="Simple AI Phone Agent", layout="wide")

# Initialize session state
if 'call_simulated' not in st.session_state:
    st.session_state.call_simulated = False
...
SAMPLE_CONVERSATIONS = {
    ...
    "Appointment Scheduling": {
        "caller": "I need to schedule an appointment for next week.",
        "ai": "I'd be happy to help you schedule an appointment. What day and time works best for you?",
        "caller_2": "How about Tuesday at 2 PM?",
        "ai_2": "Perfect! I've scheduled your appointment for Tuesday at 2:00 PM. You'll receive a confirmation email with calendar invite and meeting details. We'll also send a reminder 24 hours before. Is there anything else I can assist with?",
        "outcome": "Resolved"
    }
}
...
def analyze_call(call_type):
    ...
    return call_type, duration, sentiment_type, sentiment_info, resolution, conversation, fig_sentiment
...
def generate_call_center_analytics():
    ...
    return total_calls, intent_counts, fig_volume, fig_resolution, fig_intent
...
# Tabs
tab1, tab2 = st.tabs(["📞 Call Simulation", "📊 Analytics Dashboard"])
...
with tab1:
    ...
    call_type = st.selectbox("Select Call Type", list(CALL_INTENTS.keys()), index=0)
    
    if st.button("📞 Simulate Call", type="primary", use_container_width=True):
        st.session_state.call_simulated = True
        st.session_state.call_type = call_type
    
    if st.session_state.call_simulated:
        call_type, duration, sentiment_type, sentiment_info, resolution, conversation, fig_sentiment = analyze_call(st.session_state.call_type)
        
        st.markdown(f'<div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 20px; padding: 32px; box-shadow: 0 8px 20px rgba(102, 126, 234, 0.3); margin-bottom: 25px;"><h2 style="color: white; font-size: 32px; font-weight: 900; margin: 0 0 20px 0;">📞 Call Analysis</h2><div style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 15px;"><div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border-radius: 16px; padding: 22px; text-align: center; border: 2px solid rgba(255,255,255,0.2);"><p style="font-size: 14px; color: rgba(255,255,255,0.8); margin: 0 0 10px 0; font-weight: 600;">Intent</p><p style="font-size: 18px; color: white; font-weight: 900; margin: 0;">{call_type}</p></div><div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border-radius: 16px; padding: 22px; text-align: center; border: 2px solid rgba(255,255,255,0.2);"><p style="font-size: 14px; color: rgba(255,255,255,0.8); margin: 0 0 10px 0; font-weight: 600;">Duration</p><p style="font-size: 32px; color: white; font-weight: 900; margin: 0;">{duration:.1f}m</p></div><div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border-radius: 16px; padding: 22px; text-align: center; border: 2px solid rgba(255,255,255,0.2);"><p style="font-size: 14px; color: rgba(255,255,255,0.8); margin: 0 0 10px 0; font-weight: 600;">Sentiment</p><p style="font-size: 36px; color: white; margin: 0;">{sentiment_info["emoji"]}</p><p style="font-size: 13px; color: rgba(255,255,255,0.7); margin: 8px 0 0 0; text-transform: capitalize;">{sentiment_type}</p></div><div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border-radius: 16px; padding: 22px; text-align: center; border: 2px solid rgba(255,255,255,0.2);"><p style="font-size: 14px; color: rgba(255,255,255,0.8); margin: 0 0 10px 0; font-weight: 600;">Resolution</p><p style="font-size: 32px; color: {"#86efac" if resolution else "#fca5a5"}; font-weight: 900; margin: 0;">{"✓" if resolution else "✗"}</p><p style="font-size: 13px; color: rgba(255,255,255,0.7); margin: 8px 0 0 0;">{conversation["outcome"]}</p></div><div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border-radius: 16px; padding: 22px; text-align: center; border: 2px solid rgba(255,255,255,0.2);"><p style="font-size: 14px; color: rgba(255,255,255,0.8); margin: 0 0 10px 0; font-weight: 600;">CSAT Score</p><p style="font-size: 32px; color: #fbbf24; font-weight: 900; margin: 0;">{random.randint(4, 5)}/5</p></div></div></div>', unsafe_allow_html=True)
        
        transcript_html = f'<div style="background: linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 100%); border: 3px solid #3b82f6; border-radius: 20px; padding: 28px; box-shadow: 0 8px 20px rgba(59, 130, 246, 0.2); margin-bottom: 25px;"><h3 style="color: #1e40af; font-size: 26px; font-weight: 900; margin: 0 0 20px 0;">🗣️ Call Transcript</h3><div style="background: white; border-radius: 14px; padding: 20px; margin-bottom: 15px; box-shadow: 0 2px 6px rgba(0,0,0,0.08); border-left: 5px solid #3b82f6;"><div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;"><div style="background: #3b82f6; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center;"><span style="font-size: 20px;">👤</span></div><div><p style="font-size: 14px; color: #1e40af; font-weight: 700; margin: 0;">Caller</p><p style="font-size: 12px; color: #6b7280; margin: 0;">0:00</p></div></div><p style="font-size: 15px; color: #1f2937; margin: 0; line-height: 1.6;">"{conversation["caller"]}"</p></div><div style="background: white; border-radius: 14px; padding: 20px; margin-bottom: 15px; box-shadow: 0 2px 6px rgba(0,0,0,0.08); border-left: 5px solid #10b981;"><div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;"><div style="background: #10b981; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center;"><span style="font-size: 20px;">🤖</span></div><div><p style="font-size: 14px; color: #059669; font-weight: 700; margin: 0;">AI Agent</p><p style="font-size: 12px; color: #6b7280; margin: 0;">0:05</p></div></div><p style="font-size: 15px; color: #1f2937; margin: 0; line-height: 1.6;">"{conversation["ai"]}"</p></div><div style="background: white; border-radius: 14px; padding: 20px; margin-bottom: 15px; box-shadow: 0 2px 6px rgba(0,0,0,0.08); border-left: 5px solid #3b82f6;"><div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;"><div style="background: #3b82f6; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center;"><span style="font-size: 20px;">👤</span></div><div><p style="font-size: 14px; color: #1e40af; font-weight: 700; margin: 0;">Caller</p><p style="font-size: 12px; color: #6b7280; margin: 0;">0:{duration*0.3:.0f}</p></div></div><p style="font-size: 15px; color: #1f2937; margin: 0; line-height: 1.6;">"{conversation["caller_2"]}"</p></div><div style="background: white; border-radius: 14px; padding: 20px; box-shadow: 0 2px 6px rgba(0,0,0,0.08); border-left: 5px solid #10b981;"><div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;"><div style="background: #10b981; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center;"><span style="font-size: 20px;">🤖</span></div><div><p style="font-size: 14px; color: #059669; font-weight: 700; margin: 0;">AI Agent</p><p style="font-size: 12px; color: #6b7280; margin: 0;">0:{duration*0.6:.0f}</p></div></div><p style="font-size: 15px; color: #1f2937; margin: 0; line-height: 1.6;">"{conversation["ai_2"]}"</p></div><div style="background: linear-gradient(135deg, #d1fae5 0%, #a7f3d0 100%); border: 2px solid #10b981; border-radius: 12px; padding: 18px; margin-top: 20px; text-align: center;"><p style="font-size: 16px; color: #065f46; font-weight: 800; margin: 0;">✅ Call Ended • {conversation["outcome"]} • Duration: {duration:.1f} minutes</p></div></div>'
        
        st.markdown(transcript_html, unsafe_allow_html=True)
        
        insights_html = f'<div style="background: linear-gradient(135deg, #f3e8ff 0%, #e9d5ff 100%); border: 3px solid #a855f7; border-radius: 20px; padding: 28px; box-shadow: 0 8px 20px rgba(168, 85, 247, 0.2); margin-bottom: 25px;"><h3 style="color: #6b21a8; font-size: 26px; font-weight: 900; margin: 0 0 20px 0;">🎯 Call Insights</h3><div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px;"><div style="background: white; border-radius: 14px; padding: 22px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);"><h4 style="color: #1f2937; font-size: 18px; font-weight: 800; margin: 0 0 15px 0;">🎭 Sentiment Analysis</h4><div style="margin-bottom: 15px;"><div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;"><span style="font-size: 14px; color: #6b7280; font-weight: 600;">Overall Sentiment</span><span style="font-size: 24px;">{sentiment_info["emoji"]}</span></div><div style="background: #e5e7eb; border-radius: 12px; height: 12px; overflow: hidden;"><div style="background: {sentiment_info["color"]}; height: 100%; width: {sentiment_info["score"]*100}%; transition: width 0.3s;"></div></div><p style="font-size: 13px; color: #6b7280; margin: 8px 0 0 0; text-align: right;">{sentiment_info["score"]:.0%} {sentiment_type}</p></div><div style="background: #f9fafb; border-radius: 10px; padding: 12px;"><p style="font-size: 13px; color: #6b7280; margin: 0 0 6px 0;">Caller Tone</p><p style="font-size: 16px; color: #1f2937; font-weight: 700; margin: 0; text-transform: capitalize;">{sentiment_type}</p></div></div><div style="background: white; border-radius: 14px; padding: 22px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);"><h4 style="color: #1f2937; font-size: 18px; font-weight: 800; margin: 0 0 15px 0;">📊 Performance Metrics</h4><div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px;"><div style="background: #f0f9ff; border-radius: 10px; padding: 14px; text-align: center;"><p style="font-size: 12px; color: #6b7280; margin: 0 0 6px 0;">First Response</p><p style="font-size: 24px; color: #3b82f6; font-weight: 900; margin: 0;">5s</p></div><div style="background: #fef3c7; border-radius: 10px; padding: 14px; text-align: center;"><p style="font-size: 12px; color: #6b7280; margin: 0 0 6px 0;">Avg Response</p><p style="font-size: 24px; color: #f59e0b; font-weight: 900; margin: 0;">8s</p></div><div style="background: #f3e8ff; border-radius: 10px; padding: 14px; text-align: center;"><p style="font-size: 12px; color: #6b7280; margin: 0 0 6px 0;">Turns</p><p style="font-size: 24px; color: #a855f7; font-weight: 900; margin: 0;">4</p></div><div style="background: #d1fae5; border-radius: 10px; padding: 14px; text-align: center;"><p style="font-size: 12px; color: #6b7280; margin: 0 0 6px 0;">Resolution</p><p style="font-size: 24px; color: #10b981; font-weight: 900; margin: 0;">{"✓" if resolution else "✗"}</p></div></div></div></div></div>', unsafe_allow_html=True)
        
        ai_perf_html = '<div style="background: linear-gradient(135deg, #d1fae5 0%, #a7f3d0 100%); border: 3px solid #10b981; border-radius: 20px; padding: 28px; box-shadow: 0 8px 20px rgba(16, 185, 129, 0.2);"><h3 style="color: #065f46; font-size: 24px; font-weight: 900; margin: 0 0 18px 0;">🤖 AI Agent Performance</h3><div style="background: white; border-radius: 12px; padding: 20px; margin-bottom: 15px;"><h4 style="color: #1f2937; font-size: 18px; font-weight: 800; margin: 0 0 12px 0;">Capabilities Demonstrated</h4><div style="display: flex; flex-wrap: wrap; gap: 10px;"><span style="background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); color: white; padding: 8px 18px; border-radius: 20px; font-size: 13px; font-weight: 700; box-shadow: 0 2px 6px rgba(59, 130, 246, 0.3);">Intent Classification</span><span style="background: linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%); color: white; padding: 8px 18px; border-radius: 20px; font-size: 13px; font-weight: 700; box-shadow: 0 2px 6px rgba(139, 92, 246, 0.3);">Empathy Detection</span><span style="background: linear-gradient(135deg, #ec4899 0%, #db2777 100%); color: white; padding: 8px 18px; border-radius: 20px; font-size: 13px; font-weight: 700; box-shadow: 0 2px 6px rgba(236, 72, 153, 0.3);">Account Lookup</span><span style="background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); color: white; padding: 8px 18px; border-radius: 20px; font-size: 13px; font-weight: 700; box-shadow: 0 2px 6px rgba(245, 158, 11, 0.3);">Problem Solving</span><span style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: white; padding: 8px 18px; border-radius: 20px; font-size: 13px; font-weight: 700; box-shadow: 0 2px 6px rgba(16, 185, 129, 0.3);">Natural Language</span></div></div><div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px;"><div style="background: white; border-radius: 12px; padding: 18px; text-align: center;"><p style="font-size: 13px; color: #6b7280; margin: 0 0 8px 0;">Response Quality</p><p style="font-size: 32px; color: #10b981; font-weight: 900; margin: 0;">A+</p><p style="font-size: 12px; color: #9ca3af; margin: 8px 0 0 0;">Professional & Clear</p></div><div style="background: white; border-radius: 12px; padding: 18px; text-align: center;"><p style="font-size: 13px; color: #6b7280; margin: 0 0 8px 0;">Accuracy</p><p style="font-size: 32px; color: #3b82f6; font-weight: 900; margin: 0;">98%</p><p style="font-size: 12px; color: #9ca3af; margin: 8px 0 0 0;">Correct Information</p></div><div style="background: white; border-radius: 12px; padding: 18px; text-align: center;"><p style="font-size: 13px; color: #6b7280; margin: 0 0 8px 0;">Empathy Score</p><p style="font-size: 32px; color: #ec4899; font-weight: 900; margin: 0;">9.2/10</p><p style="font-size: 12px; color: #9ca3af; margin: 8px 0 0 0;">Highly Empathetic</p></div></div></div>'
...
with tab2:
    ...
    if st.session_state.analytics_loaded:
        total_calls, intent_counts, fig_volume, fig_resolution, fig_intent = generate_call_center_analytics()
        
        dashboard_html = '<div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 20px; padding: 32px; box-shadow: 0 8px 20px rgba(102, 126, 234, 0.3); margin-bottom: 25px;"><h2 style="color: white; font-size: 32px; font-weight: 900; margin: 0 0 20px 0;">📊 Call Center Performance</h2><div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px;"><div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border-radius: 16px; padding: 22px; text-align: center; border: 2px solid rgba(255,255,255,0.2);"><p style="font-size: 14px; color: rgba(255,255,255,0.8); margin: 0 0 10px 0; font-weight: 600;">Total Calls</p><p style="font-size: 48px; color: white; font-weight: 900; margin: 0; text-shadow: 0 2px 4px rgba(0,0,0,0.2);">8,547</p><p style="font-size: 13px; color: rgba(255,255,255,0.7); margin: 8px 0 0 0;">Last 24 hours</p></div><div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border-radius: 16px; padding: 22px; text-align: center; border: 2px solid rgba(255,255,255,0.2);"><p style="font-size: 14px; color: rgba(255,255,255,0.8); margin: 0 0 10px 0; font-weight: 600;">Resolution Rate</p><p style="font-size: 48px; color: #86efac; font-weight: 900; margin: 0; text-shadow: 0 2px 4px rgba(0,0,0,0.2);">91.5%</p><p style="font-size: 13px; color: rgba(255,255,255,0.7); margin: 8px 0 0 0;">7,821 resolved</p></div><div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border-radius: 16px; padding: 22px; text-align: center; border: 2px solid rgba(255,255,255,0.2);"><p style="font-size: 14px; color: rgba(255,255,255,0.8); margin: 0 0 10px 0; font-weight: 600;">Avg Handle Time</p><p style="font-size: 48px; color: white; font-weight: 900; margin: 0; text-shadow: 0 2px 4px rgba(0,0,0,0.2);">3.8m</p><p style="font-size: 13px; color: rgba(255,255,255,0.7); margin: 8px 0 0 0;">vs 6.2m human</p></div><div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border-radius: 16px; padding: 22px; text-align: center; border: 2px solid rgba(255,255,255,0.2);"><p style="font-size: 14px; color: rgba(255,255,255,0.8); margin: 0 0 10px 0; font-weight: 600;">CSAT Score</p><p style="font-size: 48px; color: #fbbf24; font-weight: 900; margin: 0; text-shadow: 0 2px 4px rgba(0,0,0,0.2);">4.6/5</p><p style="font-size: 13px; color: rgba(255,255,255,0.7); margin: 8px 0 0 0;">92% satisfied</p></div></div></div>'
        ...
        for idx, (intent, data) in enumerate(CALL_INTENTS.items()):
            ...
            card = f'<div style="background: white; border-left: 5px solid {intent_colors[idx]}; border-radius: 12px; padding: 18px; box-shadow: 0 2px 6px rgba(0,0,0,0.08);"><div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;"><div><span style="font-size: 18px; color: #1f2937; font-weight: 800;">{intent}</span><p style="font-size: 13px; color: #6b7280; margin: 4px 0 0 0;">Avg: {data["avg_duration"]:.1f}m • Resolution: {data["resolution_rate"]:.0%}</p></div><div style="text-align: right;"><p style="font-size: 28px; color: {intent_colors[idx]}; font-weight: 900; margin: 0;">{count:,}</p><p style="font-size: 12px; color: #6b7280; margin: 4px 0 0 0;">{pct:.1f}%</p></div></div><div style="background: #e5e7eb; border-radius: 8px; height: 8px; overflow: hidden;"><div style="background: {intent_colors[idx]}; height: 100%; width: {pct}%; transition: width 0.3s;"></div></div></div>'
            intent_cards.append(card)
        
        all_intent_cards = ''.join(intent_cards)
        
        intent_html = f'<div style="background: linear-gradient(135deg, #fef3c7 0%, #fde68a 100%); border: 3px solid #f59e0b; border-radius: 20px; padding: 28px; box-shadow: 0 8px 20px rgba(245, 158, 11, 0.2); margin-bottom: 25px;"><h3 style="color: #92400e; font-size: 26px; font-weight: 900; margin: 0 0 20px 0;">🎯 Call Intent Distribution</h3><div style="display: grid; gap: 12px;">{all_intent_cards}</div></div>'
...
st.markdown("""
    ...
    <div style="text-align: center; padding: 40px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 20px; box-shadow: 0 12px 28px rgba(102, 126, 234, 0.35); color: white;">
        ...
        <p style="font-size: 15px; margin: 18px 0; font-weight: 700;">
            <strong style="color: white;">Tech Stack:</strong> Python • Streamlit • Plotly • NLP • Voice AI
        </p>
