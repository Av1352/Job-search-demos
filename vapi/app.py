...
def simulate_voice_agent(agent_type, num_calls):
    ...
    total_cost = num_calls * 0.08  # $0.08 per call
    ...
    <div style="background: linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 100%); border: 3px solid #3b82f6; border-radius: 20px; padding: 28px; box-shadow: 0 8px 20px rgba(59, 130, 246, 0.2); margin-bottom: 25px;">
        ...
        <div style="background: white; border-radius: 14px; padding: 22px; margin-bottom: 15px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
            ...
            <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                {''.join([f'<span style="background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); color: white; padding: 8px 16px; border-radius: 16px; font-size: 13px; font-weight: 700; box-shadow: 0 2px 6px rgba(59, 130, 246, 0.3);">{uc}</span>' for uc in agent['use_cases']])}
            </div>
        </div>
    </div>
    """
    ...
    return summary_html + details_html + metrics_html, fig_success, fig_latency
...
with gr.Blocks(css=custom_css, theme=gr.themes.Soft()) as demo:
    ...
    with gr.Tabs():
        with gr.Tab("🎙️ Agent Performance"):
            gr.HTML("""
            ...
            num_calls = gr.Slider(
                ...
                value=1000,
                step=100,
                label="Number of Calls to Simulate"
            )
            
            simulate_btn = gr.Button("📞 Run Call Simulation", variant="primary", size="lg")
            
            simulation_output = gr.HTML(label="Performance Results")
            success_chart = gr.Plot(label="Success Rate Trend")
            latency_chart = gr.Plot(label="Latency Distribution")
            
            simulate_btn.click(
                fn=simulate_voice_agent,
                inputs=[agent_dropdown, num_calls],
                outputs=[simulation_output, success_chart, latency_chart]
            )
        ...
        with gr.Tab("📚 API Docs"):
            ...
            api_dropdown = gr.Dropdown(
                choices=list(API_EXAMPLES.keys()),
                value="Create Agent",
                label="Select API Endpoint"
            )
            
            show_docs_btn = gr.Button("📖 View Code Example", variant="primary", size="lg")
            
            docs_output = gr.HTML(label="API Documentation")
    ...
    <div style="text-align: center; padding: 40px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 20px; box-shadow: 0 12px 28px rgba(102, 126, 234, 0.35); color: white;">
        ...
        <p style="font-size: 15px; margin: 18px 0; font-weight: 700;">
            <strong style="color: white;">Tech Stack:</strong> Python • Gradio • Voice AI API • Real-Time Analytics
        </p>
