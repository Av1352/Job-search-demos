...
def optimize_routing(amount, method):
    ...
    # Build alternative options HTML
    alt_options = []
    for alt in alternatives:
        ...
        <div style="background: white; border-radius: 10px; padding: 16px; display: flex; justify-content: space-between; align-items: center;">
            ...
            <div style="text-align: right;">
                ...
                <p style="font-size: 12px; color: {'#10b981' if savings > 0 else '#ef4444'}; margin: 4px 0 0 0;">
                    {'Save' if savings > 0 else 'Cost'} ${abs(savings):.2f}
                </p>
            </div>
        </div>
        """
        alt_options.append(alt_html.replace('\n', '').replace('  ', ''))
...
with tab1:
    ...
    # Build method performance cards
    ...
    for idx, (method, rate) in enumerate(success_by_method.items()):
        ...
        """
        method_cards.append(card_html.replace('\n', '').replace('  ', ''))
